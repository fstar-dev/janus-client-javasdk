...
public class WebRTCSignalingApp {
    ...
    public static void main(String[] args) {
    	...
    	Javalin app = Javalin.create(config ->
                config.staticFiles.add("/main/resources/public", Location.CLASSPATH)
        );
        
        // SSL Context Factory for HTTPS configuration
        SslContextFactory.Server sslContextFactory = new SslContextFactory.Server();
//        sslContextFactory.setKeyStorePath("c:\\Program Files\\nginx-1.26.1\\nginx-1.26.1\\keystore.jks");
        sslContextFactory.setKeyStorePath("/main/resources/keystore.jks");
        sslContextFactory.setKeyStorePassword("QWE@#$qwe234");
        
        sslContextFactory.setKeyStorePath("/main/resources/keystore.jks");
    	sslContextFactory.setKeyStorePassword("QWE@#$qwe234");
    	sslContextFactory.setKeyManagerPassword("QWE@#$qwe234");
        ...
        MessageHandler messageHandler = new MessageHandler(System.getProperty("janus_address", "ws://49.13.235.28:8188"));
    ...
    private static void configureServer(JettyConfig config) {
        config.server(() -> {
            // Create a basic Javalin Jetty server
